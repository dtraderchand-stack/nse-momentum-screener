        ...
        if (quotesData.s === 'ok' && quotesData.d) {
            for (let item of quotesData.d) {
                ...
                try {
                    ...
                    const historyRes = await fetch(`https://api-t1.fyers.in/data/history?symbol=${item.symbol}&resolution=1&date_format=1&range_from=${today}&range_to=${today}`, {
                        method: 'GET',
                        headers: {
                            'Authorization': `${appId}:${access_token}`
                        }
